...
if __name__ == '__main__':
    ...
    for index in tqdm(range(len(test_list))):
        ...
        # t = entry['t_gt']
        #
        R, t = weighted_procrustes(torch.from_numpy(src_markers).double().cuda(), torch.from_numpy(tgt_markers).double().cuda())
        R = R.cpu().numpy()
    ...
    np.savez("/media/yzx/yzx_store1/Task03_Liver/Train_Test/Dataset/Deform_mesh_npz_test/stat_svd.npz", deform=deform, vis=vis)
